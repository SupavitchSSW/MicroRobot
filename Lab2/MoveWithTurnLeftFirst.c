...
#define baseSpeed 28 //first test with 28 and 0.0885
...
while((getMotorEncoder(leftMotor) <= blockDistance) || (getMotorEncoder(rightMotor) <= blockDistance)) {
      ...
      //forceToStraight();
       ...
       else if(left_dis <= 6 || right_dis <= 6){
           jane = 0.235;
           janeBoth = 0.225;
       }
       else if( left_dis <= 7  || right_dis <= 7 ) {
           jane = 0.152;
           janeBoth = 0.142;
       }
...
void moveUntilCollis(){
		front_dis 	= getUSDistance(frontUltra);
	while(front_dis<baseFrontCollis || front_dis>=250){
		front_dis 	= getUSDistance(frontUltra);
